...
from .stamps.amplitudes import stamp_t1 
...
def stamp_solver(
    ...
    part_idx = jnp.where(is_p)[0]
    hole_idx = jnp.where(~is_p)[0]
    
    nstat = len(mask_p)
    local_map = jnp.zeros(nstat, dtype=jnp.int64)
    local_map = local_map.at[part_idx].set(jnp.arange(len(part_idx)))
    local_map = local_map.at[hole_idx].set(jnp.arange(len(hole_idx)))
    for step in range(maxSteps):
        t1_new = stamp_t1(
            t1,
            t2,
